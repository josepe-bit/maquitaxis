-- Migration: 20260918010000_update_alerts_rpc_service_id.sql
-- Description: Standardize public.get_vehicle_event_alerts() RPC function to use public.get_user_servicio_id() for Nivel 2.

DROP FUNCTION IF EXISTS public.get_vehicle_event_alerts();
...
BEGIN
  ...
  ELSIF v_user_role = 'NIVEL_2' OR EXISTS (
    ...
    SELECT v.id, v.plate, v.model, v.owner_id, v.driver_id, v.servicio_id,
           v.soat_expiration_date, v.tecnomecanica_expiration_date, v.operation_card_validity_end
    FROM public.vehiculos v
    WHERE v.servicio_id = public.get_user_servicio_id();
  ...
  FOR rec_veh IN SELECT * FROM tmp_auth_vehiculos LOOP
    ...
    LOOP
      ...
      SELECT c.id::text, c.current_mileage, c.next_change_mileage, c.date, c.next_change_date INTO rec_ctrl
      ...
      IF rec_ctrl.id IS NOT NULL THEN
        ...
        v_last_km := rec_ctrl.current_mileage;
        v_last_date := rec_ctrl.date;
      ELSE
      ...
      IF v_anchor IS NULL OR (v_targ_km IS NULL AND v_targ_date IS NULL) THEN
        ...
        last_change_mileage := v_last_km;
        last_change_date := v_last_date;
        RETURN NEXT;
        CONTINUE;
      END IF;
